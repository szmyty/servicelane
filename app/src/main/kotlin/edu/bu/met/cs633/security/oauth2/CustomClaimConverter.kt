...
import edu.bu.met.cs633.security.CLAIMS_NAMESPACE
...
class CustomClaimConverter(
    ...
    override fun convert(claims: Map<String, Any>): Map<String, Any>? {
        ...
        if (attributes is ServletRequestAttributes) {
            ...
            // Add custom claims
            if (user != null) {
                convertedClaims?.set("preferred_username", user.get("preferred_username").asText())
                if (user.has("given_name")) {
                    convertedClaims?.set("given_name", user.get("given_name").asText())
                }
                if (user.has("family_name")) {
                    convertedClaims?.set("family_name", user.get("family_name").asText())
                }
                if (user.has("email")) {
                    convertedClaims?.set("email", user.get("email").asText())
                }
                // Allow full name in a name claim - happens with Auth0
                if (user.has("name")) {
                    val name = user.get("name").asText().split("\\s+".toRegex()).toTypedArray()
                    if (name.isNotEmpty()) {
                        convertedClaims?.set("given_name", name[0])
                        convertedClaims?.set("family_name", "${name.copyOfRange(1, name.size).joinToString(" ")}")
                    }
                }

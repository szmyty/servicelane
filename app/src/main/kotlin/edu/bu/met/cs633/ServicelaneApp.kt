...
import edu.bu.met.cs633.config.ApplicationProperties
import edu.bu.met.cs633.config.CRLFLogConverter
...
class ServicelaneApp(private val env: Environment) {
    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            val env = runApplication<ServicelaneApp>(*args) { DefaultProfileUtil.addDefaultProfile(this) }.environment
            logApplicationStartup(env)
        }

        @JvmStatic
        private fun logApplicationStartup(env: Environment) {
            val log = LoggerFactory.getLogger(ServicelaneApp::class.java)
